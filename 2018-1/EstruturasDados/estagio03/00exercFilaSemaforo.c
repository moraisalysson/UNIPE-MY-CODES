...
#include <stdlib.h>
#include <string.h>
#include <time.h> //para o srand e o rand
...
t_carro receberPlaca() {
    ...
    while(1) {
        printf("Informe a placa do carro (ex.: bin0101): ");
        scanf("%s", placa);
    ...
    strcpy(novo_carro.placa, strupr(placa)); //copia a placa formatada e com tudo maiúsculo
...
void statusTransito(t_fila * filaA, t_fila * filaB, int semafA, int semafB) {
    printf("--------------------------------------------\n");
    printf("          STATUS DO TRANSITO\n");
    printf("--------------------------------------------\n\n");

    printf(semafA == 1 ? "     Sinal A: ABERTO\n" : "     Sinal A: FECHADO\n");
    printf(semafB == 1 ? "     Sinal B: ABERTO\n" : "     Sinal B: FECHADO\n");

    printf("     Quantidade de carros no semaforo A: %d\n", filaA->quant_elementos);
    printf("     Quantidade de carros no semaforo B: %d\n", filaB->quant_elementos);
...
int main() {
    int opcao = 0, i = 0, semaforoA = 1, semaforoB = 0; //semáforo: 0 (fechado), 1 (aberto)
    ...
    while(1) {
        for(i = 0; i < 10; i++) {
            ...
            printf("Contador iteracoes: %d\n\n", i + 1);

            opcao = rand() % 3; //sorteia inteiro entre 0 e 2
            ...
            if(semaforoA == 1) {//se sinal A aberto, sai um carro
            ...
            else if(semaforoB == 1) { //se sinal B aberto, sai um carro
        ...
        trocaSemaforos(&semaforoA, &semaforoB); //executa após a 10ª iteração do for
    }
